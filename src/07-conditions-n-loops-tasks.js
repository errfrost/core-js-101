...
function isInsideCircle(circle, point) {
  const s = Math.sqrt((circle.center.x - point.x) ** 2 + (circle.center.y - point.y) ** 2);
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const lBracket = isStartIncluded ? '[' : '(';
  const rBracket = isEndIncluded ? ']' : ')';
  if (b > a) return `${lBracket}${a}, ${b}${rBracket}`;
  return `${lBracket}${b}, ${a}${rBracket}`;
}
...
function isCreditCardNumber(ccn) {
  const sCCN = String(ccn);
  let sum = 0;
  const parity = sCCN.length % 2;
  for (let i = 1; i <= sCCN.length; i += 1) {
    if (i % 2 === parity) sum += Number(sCCN[i - 1]);
    else if (Number(sCCN[i - 1]) > 4) sum += 2 * Number(sCCN[i - 1]) - 9;
    else sum += 2 * Number(sCCN[i - 1]);
  }
  return sum % 10 === 0;
}
...
function getDigitalRoot(num) {
  let sum = String(num).split('').map((i) => Number(i)).reduce((acc, cur) => acc + cur);
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    if (lBracket.includes(arr[i])) stack.push(arr[i]);
    else if (rBracket.includes(arr[i])) {
      if (stack.length === 0) return false;
      if (lBracket.indexOf(stack[stack.length - 1]) === rBracket.indexOf(arr[i])) stack.pop();
    }
  }
...
function getCommonDirectoryPath(pathes) {
  let res = '';
  for (let i = 0; i < Math.min(...pathes.map((l) => l.length)); i += 1) {
    const set = new Set();
    for (let j = 0; j < pathes.length; j += 1) set.add(pathes[j].split('')[i]);
    if (set.size === 1) res += [...set][0];
    else break;
  }
